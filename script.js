/** GYMPRO ELITE V11.3.0 - Calendar View + Drawer Implementation */
let state = {
...
const StorageManager = {
    ...
    getArchive() { let h = localStorage.getItem(this.KEY_ARCHIVE); try { return JSON.parse(h) || []; } catch { return []; } },
...
const exerciseDatabase = [
    ...
    { name: "Lateral Raises", muscles: ["כתפיים"], sets: [{w: 12.5, r: 13}, {w: 12.5, r: 13}], step: 0.5 },
    { name: "Weighted Pull Ups", muscles: ["גב"], sets: [{w: 0, r: 8}], step: 5, isBW: true },
    { name: "Face Pulls", muscles: ["כתפיים"], sets: [{w: 40, r: 13}], step: 2.5 },
    { name: "Barbell Shrugs", muscles: ["כתפיים"], sets: [{w: 140, r: 11}], step: 5 },
    { name: "Bench Press (Main)", muscles: ["חזה"], isCalc: true, baseRM: 100, rmRange: [80, 150], manualRange: {base: 85, min: 60, max: 140, step: 2.5} },
    { name: "Incline Bench Press", muscles: ["חזה"], sets: [{w: 65, r: 9}], step: 2.5 },
    { name: "Dumbbell Peck Fly", muscles: ["חזה"], sets: [{w: 14, r: 11}], step: 2 },
    { name: "Leg Press", muscles: ["רגליים"], sets: [{w: 280, r: 8}], step: 5 },
...
const armExercises = {
    biceps: [{ name: "Dumbbell Bicep Curls", sets: [{w: 12, r: 8}], step: 0.5 }],
...
const variationMap = { 'C': { 2: ["Dumbbell Peck Fly", "Machine Peck Fly", "Cable Fly"] } };
...
function showVariationSelect() {
    ...
    variationMap[state.type][state.exIdx].forEach(n=>{
...
function initPickers() {
    ...
    document.getElementById('btn-warmup').style.display=(state.setIdx===0 && ["Squat","Bench Press (Main)"].includes(state.currentExName))?'block':'none';
...
// --- ARCHIVE & CALENDAR ENGINE ---

function openArchive() {
    selectedArchiveIds.clear();
    switchArchiveView(state.archiveView);
    navigate('ui-archive');
}
...
function renderCalendar() {
    ...
    for(let day=1; day<=daysInMonth; day++) {
        ...
        dayEl.className = 'calendar-day' + (workoutsOnDay.length ? ' has-workout' : '');
        ...
        if(workoutsOnDay.length) {
            ...
            workoutsOnDay.forEach(w => {
                ...
                m.className = `marker type-${(w.type || 'freestyle').toLowerCase()}`;
                markers.appendChild(m);
            });
            dayEl.appendChild(markers);
            dayEl.onclick = () => openDrawer(dateStr, workoutsOnDay);
        }
...
function openDrawer(date, workouts) {
    ...
    workouts.forEach(w => {
        ...
        b.onclick = () => { closeDrawer(); showArchiveDetail(w); };
        list.appendChild(b);
    });
